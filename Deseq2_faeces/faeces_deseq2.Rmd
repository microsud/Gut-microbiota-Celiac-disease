...
ps.st1.fdr.dc.0 = subset_taxa(ps.st1.fdr.dc.0, !is.na(Family))

ps.st1.fdr.dc <-filter_taxa(ps.st1.fdr.dc.0, function(x) sum(x > 10) > (0.10*length(x)), TRUE)
ps.st1.fdr.dc
...
data_to_write1 <-res_tax_sig1[,c("baseMean","log2FoldChange","pvalue","padj","Phylum", "Class", "Order", "Family", "Genus")]
...
write.csv(data_to_write1,paste("faecesNB_significant_",paste(levels(meta.st$Diagnosis),collapse="_vs_"),".csv",sep=""))
head(data_to_write1)

df1 <- mutate(data_to_write1, Genus, Genus= paste(Family, "-",data_to_write1$Genus ))
...
fdr.dc.st <- ggplot(df1, aes(log2FoldChange, Genus)) + geom_point(aes(color = DifferentaillyAbundant), shape = 21, size = 3) + scale_color_manual(values= c("#fdae61", "#8da0cb")) + theme(axis.text.y = element_text(face="italic")) + my_theme + geom_vline(xintercept = 0)
fdr.dc.st 

ggsave("../Figures/additional/Deseq_st_DC_FDR_st.pdf", height = 6, width = 8)
...
ps.st1.fdr.ced.0 = subset_taxa(ps.st1.fdr.ced.0, !is.na(Family))

ps.st1.fdr.ced <-filter_taxa(ps.st1.fdr.ced.0, function(x) sum(x > 10) > (0.10*length(x)), TRUE)
ps.st1.fdr.ced
...
data_to_write2 <- res_tax_sig2[,c("baseMean","log2FoldChange","pvalue","padj","Phylum", "Class", "Order", "Family", "Genus")]
...
write.csv(data_to_write2,paste("faecesNB_significant_",paste(levels(meta.st2$Diagnosis),collapse="_vs_"),".csv",sep=""))
head(data_to_write2)


df2 <- mutate(data_to_write2, Genus, Genus= paste(Family, "-",data_to_write2$Genus ))

df2$Genus <- gsub('f__', '', df2$Genus)
df2$Genus <- gsub('g__', '', df2$Genus)


fdr.ced.st <- ggplot(df2, aes(log2FoldChange, Genus)) + geom_point(aes(color = DifferentaillyAbundant), shape = 21, size = 3) + scale_color_manual(values= c("#d7191c", "#8da0cb")) + theme(axis.text.y = element_text(face="italic")) + my_theme + geom_vline(xintercept = 0)
fdr.ced.st 
ggsave("../Figures/additional/Deseq_st_CeD_FDR_st.pdf", height = 6, width = 8)
...
ps.st1.dc.ced.0 = subset_taxa(ps.st1.dc.ced.0, !is.na(Family))

ps.st1.dc.ced <-filter_taxa(ps.st1.dc.ced.0, function(x) sum(x > 10) > (0.10*length(x)), TRUE)
ps.st1.dc.ced
...
data_to_write3 <-res_tax_sig3[,c("baseMean","log2FoldChange","pvalue","padj","Phylum", "Class", "Order", "Family", "Genus")]
...
write.csv(data_to_write3,paste("faecesNB_significant_",paste(levels(meta.st3$Diagnosis),collapse="_vs_"),".csv",sep=""))
head(data_to_write3)




df3 <- mutate(data_to_write3, Genus, Genus= paste(Family, "-",data_to_write3$Genus ))

df3$Genus <- gsub('f__', '', df3$Genus)
df3$Genus <- gsub('g__', '', df3$Genus)


dc.ced.st <- ggplot(df3, aes(log2FoldChange, Genus)) + geom_point(aes(color = DifferentaillyAbundant), shape = 21, size = 3) + scale_color_manual(values= c("#d7191c", "#fdae61")) + theme(axis.text.y = element_text(face="italic")) + my_theme + geom_vline(xintercept = 0)
dc.ced.st 
ggsave("../Figures/additional/Deseq_st_CeD_DC_st.pdf", height = 6, width = 8)
#"#d7191c", "#fdae61", "#8da0cb
...
ggsave("../Figures/Fig5a_b_c_deseq_st.pdf", height = 6, width = 20)
